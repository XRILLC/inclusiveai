...
<tbody>
  ...
  <tr>
    ...
    <td>https://huggingface.co/meta-llama/Llama-3.2-3B-Instruct, https://huggingface.co/TinyLlama/TinyLlama-1.1B-step-50K-105b, https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v0.1</td>
  </tr>
  <tr>
...
Answer (TinyLlama Chat): I would use this to find images that match a specific search query.  I would use this to find images that match a specific search query.
Answer (Tiny Llama Step): *Could not answer*
...
Answer (TinyLlama Chat): The evaluation of annotation proposals is still ongoing. *The next steps include:

Answer (Tiny Llama Step): The evaluation of the proposed annotation proposals is a key step in the development of the proposed framework.
...
Another interesting note was that while implmeenting the tiny llama, and even the full llama, the questions that asked for the LLM's opinion provided no answer, or an unintelligable answer. This could be because the prompt said to pull from the text and the text isn't introspective of itself in these ways. Bur regardless still interesting.
